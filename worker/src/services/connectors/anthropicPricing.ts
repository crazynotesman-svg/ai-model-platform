/**
 * connectors/anthropicPricing.ts —— Anthropic 官方 Pricing Connector v2（Phase 11.7）
...
const NAME = 'Anthropic Pricing';
const SOURCE_URL = 'https://www.anthropic.com/pricing';

async function fetchAnthropicPricing(): Promise<unknown> {
...
async function parsePricing(raw: unknown): Promise<{ modelKey: string; inputPrice: number; outputPrice: number; effectiveDate: string }[]> {
  return []; // 解析映射待人工核验
}
...
async function normalize(raw: unknown): Promise<DataEventInput[]> {
  ...
  return rows.map((r) => ({
    ...
    entityId: r.modelKey,
    payload: { inputPrice: r.inputPrice, outputPrice: r.outputPrice, effectiveDate: r.effectiveDate, sourceUrl: SOURCE_URL },
    sourceName: NAME,
    confidence: 95,
  }));
}
...
export const anthropicPricingConnector: DataConnector = {
  name: NAME,
